...
class SX127x:
    ...
    def begin_packet(self, implicit_header_mode=False):
        ...
        # FIFO mutató alaphelyzetbe állítása
        self.write_register(REG_FIFO_ADDR_PTR, FifoTxBaseAddr)
        self.write_register(REG_PAYLOAD_LENGTH, 0)
    ...
    '''def write(self, buffer):
        ...
        return size'''
    def write(self, buffer):
        # FIFO mutató beállítása a kezdőcímre
        self.write_register(REG_FIFO_ADDR_PTR, FifoTxBaseAddr)
        ...
        # Csomag hosszának beállítása
        payload_length = len(buffer)
        self.write_register(REG_PAYLOAD_LENGTH, payload_length)
    
        # Visszaadja az írt adatok számát
    ...
    #javított bytes fogadáshoz:
    def println(self, msg, implicit_header=False):
    
        """
        Üzenet küldése a LoRa chip segítségével.
        :param msg: Az üzenet (`str` vagy `bytes`).
        :param implicit_header: Implicit fejléccel való küldés (opcionális).
        """
        self.set_lock(True)  # Zárolás a küldés alatt

        self.begin_packet(implicit_header)

        # Üzenet típusának ellenőrzése és konvertálása
        if isinstance(msg, str):
            message = msg.encode()
        elif isinstance(msg, bytes):
            message = msg
        else:
            raise ValueError("msg must be a string or bytes")

        self.write(message)  # Az üzenet elküldése

        self.end_packet()
        self.set_lock(False)  # Zárolás feloldása
        self.collect_garbage()
    ...
    def read_payload(self):
        # FIFO mutató visszaállítása az aktuális RX címre
        self.write_register(REG_FIFO_ADDR_PTR, 
                self.read_register(REG_FIFO_RX_CURRENT_ADDR)
                 )

        # Csomag hosszának olvasása
        if self._implicit_header_mode:
            packet_length = self.read_register(REG_PAYLOAD_LENGTH)
        else:
            packet_length = self.read_register(REG_RX_NB_BYTES)

        # FIFO tartalmának olvasása
        payload = bytearray()
